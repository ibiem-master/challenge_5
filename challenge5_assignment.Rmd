...
One quirk of bash chunks in Rmarkdown documents is that each bash chunk in an has it's own environment independent of the other chunks, so variables defined in one bash chunk are not available in other chunks
...
Look up how to use `Sys.setenv`, then use the R chunk below to define the bash variable `$FAVORITE_DESSERT` so that the bash chunk below runs correctly
